...
app.post('/sendMessage', (req, res) => {
  ...
  connection.then((db) => {
    db.collection('SellerMessages').insertOne(details).then((result) => {
      res.json(result);
      console.log(result);
    })
  });
...
app.get('/messageview', (req, res) => {
  connection.then((db) => {
    db.collection('SellerMessages').find({}).toArray().then((messages) => {
      res.json(messages);
    }).catch((err) => {
